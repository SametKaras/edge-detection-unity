...
namespace SceneCapture.Hough
{
    ...
    public class HoughNormalVisualizer : MonoBehaviour
    {
        ...
        [Range(0.01f, 1f)]
        public float edgeThreshold = 0.1f;              // Edge detection threshold
        
        [Header("Normal Hesaplama")]
        public bool useLocalGradient = true;            // Komşu gradientleri kullan
        [Range(2, 30)]
        public int localGradientRadius = 8;             // Komşuluk yarıçapı
        public bool fallbackToLocalGradient = true;     // Segment yoksa fallback
        
        [Header("Görselleştirme")]
        public float arrowLength = 60f;                 // Normal ok uzunluğu
        public Color segmentColor = new Color(0f, 1f, 1f, 0.5f);
        public float hoverRadius = 25f;                 // Hover mesafesi
        public bool showAllSegments = false;            // Tüm segmentleri göster
        ...
        // ==================== RENKLER ====================
        private static readonly Color COLOR_HOUGH = Color.white;                    // Hough normal (beyaz)
        private static readonly Color COLOR_5X5 = new Color(1f, 0.4f, 1f);         // 5x5 (magenta)
        private static readonly Color COLOR_7X7 = new Color(1f, 1f, 0.3f);         // 7x7 (sarı)
        private static readonly Color COLOR_WARNING = new Color(1f, 0.3f, 0.3f);   // Uyarı (kırmızı)
        private static readonly Color COLOR_OK = new Color(0.3f, 1f, 0.3f);        // OK (yeşil)
        
        // ==================== BİLEŞENLER ====================
        private Camera _camera;
        private EdgeFrameProcessor _processor;       // GPU→CPU veri transferi
        private HoughLineDetector _detector;         // Hough Transform
        
        private LineSegment2D? _hoveredSegment;      // Mouse'un üzerindeki segment
        private Vector2 _mousePos;                   // Mouse pozisyonu
        private Vector2 _currentNormal;              // Hesaplanan normal
        private bool _isInitialized;
        
        private RenderTexture _edgeSourceRT;         // Edge detection için RT
        private Texture2D _whiteTex;                 // Çizim için beyaz texture
        private Texture2D _boxTex;                   // GUI box için texture
        
        // ==================== KERNEL BUFFER'LARI ====================
        // Burst job'ların hesapladığı normal vektörleri (her pixel için)
        private NativeArray<float2> _kernel5x5Normals;  // 5x5 kernel sonuçları
        private NativeArray<float2> _kernel7x7Normals;  // 7x7 kernel sonuçları
        private bool _kernelBuffersReady;
        private int _kernelBufferWidth, _kernelBufferHeight;
        
        // ==================== İSTATİSTİKLER ====================
        private float _avgError5x5, _avgError7x7;      // Ortalama açı farkları
        private float _computeTime5x5, _computeTime7x7; // Hesaplama süreleri
        ...
        void Update()
        {
            // Runtime'da parametre değişikliklerini uygula
            _detector?.UpdateParameters(houghParams);
        }
        
        /// <summary>
        /// Her frame çağrılır - Ana render loop
        /// </summary>
        void OnRenderImage(RenderTexture src, RenderTexture dst)
        {
            if (!_isInitialized) { Graphics.Blit(src, dst); return; }
            
            // Edge source RT hazırla
            ...
            if (_processor.IsDataReady && _processor.EdgePixelCount > 0)
            {
                ...
                _detector.DetectLines(
                    ...
                    _processor.Height,
                    _processor,
                    (int)edgeDetectionKernel  // Hangi kernel kullanıldı (debug için)
                );
                
                // 5x5 ve 7x7 kernel hesaplama (karşılaştırma için)
        ...
        /// <summary>
        /// 5x5 ve 7x7 kernel ile normal hesaplama (BURST JOB)
        /// Her pixel için komşuluktaki gradient'leri toplayarak normal hesaplar
        /// </summary>
        private void ProcessKernels()
        {
            ...
            // Buffer boyutu değiştiyse yeniden oluştur
            if (!_kernelBuffersReady || _kernelBufferWidth != w || _kernelBufferHeight != h)
            {
                DisposeKernelBuffers();
                _kernel5x5Normals = new NativeArray<float2>(pixelCount, Allocator.Persistent); // Frame'ler arası kalıcı bellek
            ...
            new KernelJob {
                ...
                width = w, height = h,
                radius = 3  // 7x7 = radius 3
            }.Schedule(pixelCount, 64).Complete();
            _computeTime7x7 = (float)_stopwatch.Elapsed.TotalMilliseconds;
            
            // İstatistikleri hesapla (açı farkları)
            CalculateStatistics();
        }
        
        /// <summary>
        /// Hough normal ile kernel normallar arasındaki ortalama açı farkını hesapla
        /// </summary>
        ...
        /// <summary>
        /// İki açı arasındaki minimum farkı hesapla (0-90° arası)
        /// </summary>
        private float CalculateAngleDiff(float angle1, float angle2)
        {
            float diff = Mathf.Abs(angle1 - angle2);
            if (diff > 180) diff = 360 - diff;  // 180° üzeri flip et
            if (diff > 90) diff = 180 - diff;   // 90° üzeri flip et (normal perpendicular)
            return diff;
        }
        ...
        void OnGUI()
        {
            ...
            // Tüm segmentleri çiz (opsiyonel)
            if (showAllSegments) DrawAllSegments();
            
            // Mouse'a yakın segment bul
            FindHoveredSegment();
            ...
            {
                DrawHoveredSegment();           // Segment çiz
                DrawNormalArrows(texCoord);     // Normal okları çiz
                
                // Segment debug paneli
                if (showSegmentDebug)
                    DrawSegmentDebugPanel(texCoord);
            }
            else if (fallbackToLocalGradient && isOnEdge)
            {
                // Segment yok ama edge var → Fallback normal göster
                DrawFallbackNormals(texCoord);
            }
        }
        
        /// <summary>
        /// Sol üst status bar - Segment ve edge pixel sayıları
        /// </summary>
        ...
        /// <summary>
        /// Sağ üst stats panel - Kernel karşılaştırma istatistikleri
        /// </summary>
        ...
        /// <summary>
        /// Segment debug paneli - Detaylı segment bilgileri ve uyarılar
        /// </summary>
        private void DrawSegmentDebugPanel(Vector2Int texCoord)
        {
            ...
            // Panel içeriği
            string kernelName = seg.GetKernelName();  // Sobel/Roberts/Prewitt
            ...
            // Kernel karşılaştırma bilgileri
            ...
            // Panel pozisyonu hesapla (mouse yanı)
            ...
            // Üst border rengi (sorun durumuna göre)
        ...
        /// <summary>
        /// Normal oklarını çiz - Hough, 5x5, 7x7 karşılaştırması
        /// </summary>
        private void DrawNormalArrows(Vector2Int texCoord)
        {
            ...
            // Kernel normallar (eğer karşılaştırma aktifse)
            if (enableKernelComparison && _kernelBuffersReady)
            {
                int idx = texCoord.y * _kernelBufferWidth + texCoord.x;
                
                // 5x5 kernel normal (magenta)
        ...
        /// <summary>
        /// Mouse'a en yakın segment'i bul (hover detection)
        /// </summary>
        ...
        /// <summary>
        /// Tüm segmentleri soluk olarak çiz
        /// </summary>
        ...
        /// <summary>
        /// Hover edilen segment'i kalın çiz
        /// </summary>
        ...
        /// <summary>
        /// Fallback: Segment yoksa yerel gradient kullanarak normal hesapla
        /// </summary>
        ...
        /// <summary>
        /// Yerel gradient kullanarak normal hesapla (komşuluktan)
        /// </summary>
        private Vector2 CalculateLocalNormal(int cx, int cy, int radius)
        {
            ...
            {
                gradSum += centerGrad.normalized * 3f;  // Merkeze bonus ağırlık
                totalWeight += 3f;
            }
        ...
        /// <summary>
        /// BURST JOB 3: Kernel normal hesaplama
        /// Her pixel için komşuluktaki gradient'leri ağırlıklandırarak normal hesaplar
        /// 18x hızlanma! (80ms → 4.5ms)
        /// </summary>
        [BurstCompile]
        private struct KernelJob : IJobParallelFor
        {
            [ReadOnly] public NativeArray<float> gradX;     // Gradient X
            [ReadOnly] public NativeArray<float> gradY;     // Gradient Y
            [WriteOnly] public NativeArray<float2> normals; // Hesaplanan normallar
            
            public int width, height, radius;               // Grid boyutları ve kernel radius
            
            public void Execute(int index)
            {
                // 1D index → 2D koordinat
                int x = index % width;
                int y = index / width;
                ...
                // Komşuları tara (radius × radius pencere)
                for (int dy = -radius; dy <= radius; dy++)
                {
                    for (int dx = -radius; dx <= radius; dx++)
                    {
                        ...
                        // Komşu koordinatları (bounds check)
                        ...
                        float gyVal = -gradY[pidx];  // Y flip
                        float mag = math.sqrt(gxVal * gxVal + gyVal * gyVal);
                        
                        if (mag < 0.05f) continue;  // Çok zayıf, atla
                        
                        // Gaussian-like ağırlık (merkeze yakın = daha ağır)
                        float distWeight = 1f - math.sqrt((float)distSq) / (radius + 1f);
                        float weight = distWeight * mag;
                        
                        // Merkez piksele 3x bonus
                ...
                // Ortalama hesapla ve normal döndür
                if (weightSum > 0.1f)
                {
